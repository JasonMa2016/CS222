...
def run(FPR, tau_lst, alpha=0.6185):
	# get the data
	...
	for tau in  tau_list:
		...
		b_1, b_2 = solve_SBF_size(FPR, F_p, F_n, alpha)
		print(b_1, b_2)
		# first BF
		BF1 = BloomFilter(len(S_train), alpha ** (b_1))
		for x in S_train:
			BF1.add(x)

		BF2 = BloomFilter(len(FN_list), alpha ** (b_2/F_n))
		for x in FN_list:
			BF2.add(x)

		print("First BF size:", BF1.size / 8)
		print("Second BF size:", BF2.size / 8)
		print("Total size:", (BF1.size+BF2.size) / 8)
		fps = 0
		total = 0
		fps_adv  = 0
		total_adv = 0
